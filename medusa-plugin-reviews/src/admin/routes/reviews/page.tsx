...
const ProductReviewsPage = () => {
    ...
    const { mutate: updateReview } = useAdminCustomPost(
        "/product-review",
        ["product-review"]
    );
    ...
    return (
        <div className="p-8 shadow-elevation-card-rest bg-ui-bg-base w-full rounded-lg overflow-hidden">
            ...
            <div className="flex gap-4 mb-6 items-end justify-end">
                ...
                <div>
                    <label className="block text-sm font-medium mb-1 w-29">Minimum Rating</label>
                    <Select
                        ...
                        filteredReviews.map((review) => {
                            ...
                            return (
                                <Table.Row key={review.id} className="hover:bg-gray-100 !h-20">
                                    ...
                                    <Table.Cell>{review.variant.title || "Unknown Variant"}</Table.Cell>
                                    <Table.Cell>{review.rating}</Table.Cell>

                                    <Table.Cell className="truncate max-w-[300px]">{review.title}</Table.Cell>
                                    <Table.Cell className="max-w-[300px]">
                                        <div className=" whitespace-pre-wrap break-words">{review.content}</div>
                                    </Table.Cell>
                                    <Table.Cell>{review.customer_email || review.customer_name}</Table.Cell>
                                    <Table.Cell>
                                    ...
                                    <Table.Cell>
                                        <div className="flex flex-col gap-2">
                                            <Button
                                                ...
                                                size="small"
                                                onClick={() => handleReviewAction(review.id, toggleAction)}
                                            >
                                                {actionLabel}
                                            </Button>
                                            <Button
